...
def load_records():
    ...
    Returns:
      ...
      warnings: count of malformed/unbalanced blocks
...
def process_chunk(args):
    ...
    pat = re.compile(PAN_REGEX)

    doc = Document()
    rows = []
    seen = had_pan = hits = parse_fail = 0
    parse_errors = []
    ...
    for set_name, key, raw_json in chunk:
        ...
        for m in pat.finditer(raw_json):
            pan = m.group(1)
            if any(pan in v for v in values):
                this_hits.append((pan, m.span(1)))
        if not this_hits:
            continue
        ...
        for pan, _ in sorted(this_hits, key=lambda x: x[1][0]):
            hits += 1
            run1 = para.add_run(f" | {pan}")
            ...
            rows.append((set_name, key, raw_json, pan, field))

    if rows:
        os.makedirs(TEMP_DIR, exist_ok=True)
        doc.save(os.path.join(TEMP_DIR, f"chunk_{idx}.docx"))

    result_list.append((rows, seen, had_pan, hits, parse_fail, parse_errors))
...
def write_excel(rows, parse_errors):
    ...
    # Matches
    ws.write_row(0, 0, ["Set Name", "Key", "Full JSON", "PAN", "Field"])
    r = 1
    for a, b, c, p, f in rows:
        ws.write(r, 0, a)
        ws.write(r, 1, b)
        ws.write(r, 2, c)
        ws.write(r, 3, p, red)
...
if __name__ == "__main__":
    ...
    # Aggregate
    all_rows = []
    total_seen = total_with_pan = total_hits = total_pf = 0
    parse_errors = []
    for rows, seen, had_pan, hits, pfail, pfd in results:
        all_rows.extend(rows)
        total_seen += seen
        total_with_pan += had_pan
        total_hits += hits
        total_pf += pfail
    ...
    print(
        ...
        f"{total_with_pan} had PANs, "
        f"{total_hits} total hits, "
        f"{total_pf} JSON parse failures"
    )

    # Output
    if os.path.isdir(TEMP_DIR) and total_with_pan > 0:
        merge_word()
